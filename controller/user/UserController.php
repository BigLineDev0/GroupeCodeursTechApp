...
require_once("../newsletter/NewsletterController.php");
...
class UserController
{
    ...
    // Permet d'nevoyer un mail de confirmation
    public function sendMailConfirmUser()
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            // Validation des données
            if (empty($email)) {
                $this->setErrorAndRedirect("l'amil est obligatoire.", "Erreur de confirmation", "reinitEmail");
            }

            // Validation de l'email
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                $this->setErrorAndRedirect("L'adresse email n'est pas valide.", "Erreur de confirmation", "reinitEmail");
            }
            ...
            $_SESSION['code'] = $code;
            $_SESSION['email_user_reinit'] = $email;

            $message = "<a href='http://localhost/GroupeCodeursTechApp/reinit?code=$code&email=$email'>Merci de cliquer ici pour réinitialiser votre mot de passe</a>";
            $this->newsletterController->sendMessageMail($email, $message);

            $this->setSuccessAndRedirect("Email de réinitialisattion du mot de passe envoyé avec succès.", "Réinitialisation du mot de passe", 'reinitEmail');
            
        }
    }

    // Permet de réinitier le mot de passe
    function reinitPassword()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $code = $_SESSION['code'];
            $_SESSION['email_user_reinit'];

            // Vérifier si tous les champs sont bien remplis
            if (empty($newPassword) || empty($confirmPassword)) {
                $this->setErrorAndRedirect('Tous les champs sont obligatoires.', "Erreur de réinitialisation du mot de passe.", "reinit?code=$code");
            }

            // Validation des 2 password
            if ($newPassword != $confirmPassword) {
                $this->setErrorAndRedirect('Le nouveau mot de passe et le mot de passe de confirmaion ne correspondent pas.', "Erreur de réinitialisation du mot de passe.", "reinit?code=$code");
            }

            // Validation du newpassword
            if (strlen($newPassword) < 8) {
                $this->setErrorAndRedirect('Le nouveau mot de passe doit contenir au moins 8 caractères.', "Erreur de réinitialisation du mot de passe.", "reinit?code=$code");
            }
           
            // Vérifier un user existe deja
            try {
                $user= $this->userRepository->getUserByEmail($_SESSION['email_user_reinit']);
                if (!$user) {
                    $this->setErrorAndRedirect('Utilisateur non trouvé dans la base de données.', "Erreur de réinitialisation du mot de passe.", "reinit?code=$code");
                }

                // hasché le mot de passe
                $hashPassword = password_hash($newPassword, PASSWORD_DEFAULT);

                // changement du mot de passe dans la base de données
                $this->userRepository->updatePassword($user['id'], $hashPassword);

                // Envoi mail
                $this->sendPasswordEmail($user['nom'], $user['email'], $newPassword, "Votre mot de passe a été réinitialisé avec succès. Voici vos identifiants.");

                $this->setSuccessAndRedirect("Votre mot de passe a été réinitialisé avec succès.", "Réinitialisation Réussie", "login");

            } catch (Exception $error) {
                $this->setErrorAndRedirect("Erreur " . $error->getMessage(), "Erreur de réinitialisation du mot de passe.", "reinit?code=$code");
            }
        }
    }
